...
const AdminRTE = () => {
  ...
  const handleExport = () => {
    exportRowsToPdf({
      title: `RTE Students Report${selectedClass === 'All Classes' ? '' : ` - ${selectedClass}`}`,
      fileName: `rte-students-${selectedClass.replace(/\s+/g, '-').toLowerCase()}-${Date.now()}.pdf`,
      summaryLines: [
  ...
  return (
    <div>
      ...
      <PageHeader
        title="RTE Students"
        subtitle={selectedClass === 'All Classes' ? `${students.length} total RTE students` : `${selectedClassStats.totalStudents} RTE students in ${selectedClass}`}
      >
